...
```{r US voter turnout data, echo= TRUE}
...
```{r US broadband access data, echo= TRUE}
...
```{r Merge voter turnout and broadband data + add regions (categorical variable), echo= TRUE}

voter_broadband <- merge(broadband_data, election_comparison, by =c("STATE", "YEAR")) %>%
  merge(regions, by.x="STATE", by.y="State")
```

```{r  Final states voter turnout + broadband dataset with map data, echo= TRUE}

vb_map <- states %>%
  merge(voter_broadband, by.x="state_name", by.y="STATE")
```

```{r Clean WA counties broadband access data, echo= TRUE}
...
```{r Clean voting age population data, echo= TRUE}
...
```{r Add voter turnout rate (numerical variable), echo= TRUE}
...
```{r Heatmap: Voter Turnout Rates 2016 vs. 2020, echo= TRUE}
vb_map %>%
  ggplot(aes(long, lat)) + 
...
```{r Heatmap: Broadband Access Rates 2016 vs. 2020, echo= TRUE}
vb_map %>%
  ggplot() + 
...
```{r Scatterplot: US Broadband Access vs Voter Turnout, echo= TRUE}
...
```{r Average Turnout in Top 5 and Bottom 5 Broadband Access States, echo= TRUE}
voter_broadband %>%
  group_by(YEAR) %>%
...
```{r Bar Graph: Regional Voter Turnout Vs. Broadband Access, echo= TRUE}
...
```{r Heatmaps: Regional Voter Turnout vs Broadband Access, echo= TRUE}
vb_map %>%
  ggplot() + 
  geom_polygon(aes(long, lat, group = group, fill = Broad_access), color = "white") +
  scale_fill_gradient(low = "yellow", high = "darkblue") +
...
```{r Heatmap: WA Counties Voter Turnout, echo= TRUE}
wa_vb_map %>%
  ggplot() + 
...
```{r Heatmap: WA Counties Broadband Access, echo= TRUE}
wa_vb_map %>%
  ggplot() + 
...
```{r Average Turnout in Top 5 and Bottom 5 Broadband Access Counties, echo= TRUE}
wa_voter_broadband %>%
  group_by(year) %>%
  filter(rank(desc(Broad_access), ties.method = "first") <= 5) %>%
